...
use crate::io::Buffer;
use crate::kernel::constants::{EINVAL, ENOENT, ENOTDIR, EPERM, ERANGE, ESRCH};
...
fn tgkill(tgid: u32, tid: u32, sig: u32) -> KResult<()> {
    let process_list = Task::block_on(ProcessList::get().read());

    let thread = process_list.try_find_thread(tid).ok_or(ESRCH)?;

    if thread.process.pid != tgid {
        return Err(ESRCH);
    }

    thread.raise(Signal::try_from_raw(sig)?);
    Ok(())
}
